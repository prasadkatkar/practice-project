...
export default function ProductPage() {
  return (
    <div>
      ...
      <div className="shrink-0 justify-center gap-[54px] flex flex-wrap max-w-full mt-[40px] pb-5">
        {products.map((item) => (
          <Link
            href={`/product/${item.heading}`}
            key={item.heading}
            className="grow-0 flex flex-col items-center"
          >
            <div className="max-w-full">
              <Image src={item.image1} alt="Bed" width={400} height={300} />
            </div>
            <div className="bg-white shadow-2xl w-50 flex flex-col text-center p-3 -mt-6">
              <h1>{item.heading}</h1>
